<!-- Modal sửa thông số chiều dài dây -->
<div>
    <div x-show="{{ $modalOpen }}" x-cloak x-transition.opacity style="background-color: rgba(0,0,0,0.6);"
        ...
        <div @click.away="modalOpen = false" x-transition.scale
            ...
            <div class="flex items-center justify-between mb-5">
                ...
                <h2 class="text-2xl font-bold text-gray-100">Sửa thông số chiều dài dây</h2>
            ...
            <!-- Form -->
            <form action="{{ route('chieudaiday-sp.sua', $ts->MADD) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <label for="CHISO" class="block text-gray-200 mb-1">Chỉ số (giá trị):</label>
                    </div>
                    <div class="relative">
                        <input type="number" id="CHISO" name="CHISO" step="any"
                            value="{{ old('CHISO', $ts->CHISO) }}"
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" id="DONVIDO" name="DONVIDO"
                            value="{{ old('DONVIDO', $ts->DONVIDO) }}"
                            class="w-full pl-3 px-3 py-2 rounded bg-gray-800 text-gray-100 border border-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
                    </div>
                </div>
                ...
                <div>
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <label for="MOTA" class="block text-gray-200 mb-1">Mô tả chiều dài dây:</label>
                    </div>
                    <div class="relative">
                        <textarea id="MOTA" name="MOTA" rows="3"
                            class="w-full pl-3 px-3 py-2 rounded bg-gray-800 text-gray-100 border border-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none">{{ old('MOTA', $ts->MOTA) }}</textarea>
                    </div>
                </div>
