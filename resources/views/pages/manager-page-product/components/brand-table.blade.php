...
<tr class="hover:bg-gray-700 transition-colors">
    <td class="px-4 py-2 font-medium">{{ $th->MATHUONGHIEU }}</td>
    <td class="px-4 py-2">{{ $th->TENTHUONGHIEU }}</td>
    <td class="px-4 py-2">{{ $th->MOTA ?? '-' }}</td>
     <td class="px-4 py-2">{{ $th->QUOCGIA ?? '-' }}</td>
    <td class="px-4 py-2">{{ \Carbon\Carbon::parse($th->NGAYTAO)->format('d/m/Y H:i') }}</td>
    <td class="px-4 py-2 flex gap-2">
        ...
        <div x-data="{ modalOpen: false }">
            ...
            @include('components.message-box.brand-fix', ['thuonghieu' => $th, 'modalOpen' => 'modalOpen'])
        </div>

        <!-- Nút Xóa -->
        <form action="{{ route('thuonghieu-sp.xoa', $th->MATHUONGHIEU) }}" method="POST" class="inline">
            @csrf
            @method('DELETE')
            <button type="submit"
