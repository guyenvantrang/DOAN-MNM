@forelse($loaisps as $loai)
<tr class="hover:bg-gray-700 transition-colors">
    <td class="px-4 py-2 font-medium">{{ $loai->MALOAI }}</td>
    <td class="px-4 py-2">{{ $loai->TENLOAI }}</td>
    <td class="px-4 py-2">{{ $loai->MOTA ?? '-' }}</td>
    <td class="px-4 py-2">{{ \Carbon\Carbon::parse($loai->NGAYTAO)->format('d/m/Y H:i') }}</td>
    <td class="px-4 py-2 flex gap-2">
        ...
        <div x-data="{ modalOpen: false }">
            ...
            @include('components.message-box.category-fix', ['loai' => $loai, 'modalOpen' => 'modalOpen'])
        </div>

        <!-- Nút Xóa -->
        <form action="{{ route('loai_sp.xoa', $loai->MALOAI) }}" method="POST" class="inline">
            @csrf
            @method('DELETE')
            <button type="submit"
