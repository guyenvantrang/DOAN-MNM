@forelse($dodays as $dd)
    <tr class="hover:bg-gray-700 transition-colors">
        <td class="px-4 py-2 font-medium">{{ $dd->MADDY }}</td>
        <td class="px-4 py-2">{{ $dd->CHISO ?? '-' }}</td>
        <td class="px-4 py-2">{{ $dd->DONVIDO ?? '-' }}</td>
        <td class="px-4 py-2">{{ $dd->MOTA ?? '-' }}</td>
        <td class="px-4 py-2">{{ \Carbon\Carbon::parse($dd->NGAYTAO)->format('d/m/Y H:i') }}</td>
        <td class="px-4 py-2 flex gap-2">
            ...
            <div x-data="{ modalOpen: false }">
                ...
                @include('components.message-box.parameters-thickness-fix', ['doday' => $dd, 'modalOpen' => 'modalOpen'])
            </div>

            <!-- Nút Xóa -->
            <form action="{{ route('doday-sp.xoa', $dd->MADDY) }}" method="POST" class="inline">
                @csrf
                @method('DELETE')
                <button type="submit"
                    class="flex items-center gap-1 px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded text-sm transition"
                    onclick="return confirm('Bạn có chắc muốn xóa độ dày này không?')">
                    <!-- Icon thùng rác -->
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
