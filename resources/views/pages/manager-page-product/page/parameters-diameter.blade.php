...
@section('content')
    <div class="p-6 w-full">
        ...
        <h1 class="text-2xl font-semibold mb-6">Quản lý thông số đường kính</h1>
        ...
        <div class="flex flex-col sm:flex-row gap-3 mb-6 items-start sm:items-center">
            <div class="flex-1 flex gap-3">
                <div class="relative w-full sm:w-1/3">
                    <input type="text" id="search" 
                        data-url-search="{{ url('duongkinh/duongkinh_sp/timkiemtheomavaten') }}"
                        data-url-date="{{ url('duongkinh/duongkinh_sp/timkiemtheongay') }}"
                        data-url-all="{{ url('duongkinh/duongkinh_sp/hienthitatca') }}"
            ...
            <div x-data="{ modalOpen: false }">
                <button @click="modalOpen=true"
                    ...
                    <span>Thêm đường kính</span>
                </button>

                {{-- Modal thêm đường kính --}}
                @include('components.message-box.parameters-diameter-add')
            </div>
        </div>
        ...
        <div x-data="categoryTable()" class="overflow-x-auto shadow-md sm:rounded-lg w-full">
            <table class="w-full text-sm text-left text-gray-100">
                <thead class="text-xs uppercase bg-gray-700 text-gray-200">
                    <tr>
                        {{-- Cột Mã đường kính --}}
                        <th class="px-4 py-3 cursor-pointer select-none" data-column="MADK" data-direction="asc"
                            ...
                            <div class="inline-flex items-center gap-2 group">
                                ...
                                <span class="text-gray-200 group-hover:text-blue-400 transition-colors">Mã đường kính</span>
                ...
                <tbody id="category-table" class="bg-gray-800">
                    @include('pages.manager-page-product.components.parameters-diameter-table', ['duongkinh' => $duongkinh])
                </tbody>
            </table>
        </div>
    </div>
...
<script>
    function sortColumn(el) {
        ...
        fetch(`{{ route('duongkinh-sp.sapxeptheoid') }}?column=${column}&direction=${direction}`)
