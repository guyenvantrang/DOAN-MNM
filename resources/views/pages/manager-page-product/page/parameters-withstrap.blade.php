...
@section('content')
    <div class="p-6 w-full">
        ...
        <h1 class="text-2xl font-semibold  mb-6">Quản lý chiều rộng dây</h1>

        {{-- Tìm kiếm + lọc ngày + reset --}}
        <div class="flex flex-col sm:flex-row gap-3 mb-6 items-start sm:items-center">
            <div class="flex-1 flex gap-3">
                <div class="relative w-full sm:w-1/3">
                    <input type="text" id="search" data-url-search="{{ url('chieurongday/crd/timkiemtheomavaten') }}"
                        data-url-date="{{ url('chieurongday/crd/timkiemtheongay') }}"
                        data-url-all="{{ url('chieurongday/crd/hienthitatca') }}" placeholder="Tìm kiếm chiều rộng dây..."
            ...
            {{-- Nút mở modal thêm chiều rộng dây --}}
            <div x-data="{ modalOpen: false }">
                <button @click="modalOpen=true"
                    ...
                    <span>Thêm chiều rộng dây</span>
                </button>

                {{-- Modal thêm thông số --}}
                @include('components.message-box.parameters-withstrap-add')
            </div>
        </div>
        ...
        <div x-data="categoryTable()" class="overflow-x-auto shadow-md sm:rounded-lg w-full">
            <table class="w-full text-sm text-left text-gray-100">
                <thead class="text-xs uppercase bg-gray-700 text-gray-200">
                    <tr>
                        <th class="px-4 py-3 cursor-pointer select-none" data-column="MCRD" data-direction="asc"
                            ...
                            <div class="inline-flex items-center gap-2 group">
                                ...
                                <span class="text-gray-200 group-hover:text-blue-400 transition-colors">Mã CRD</span>
                                <span
                ...
                <tbody id="category-table" class="bg-gray-800">
                    @include('pages.manager-page-product.components.parameters-withstrap-table', ['chieurongday' => $chieurongday])
                </tbody>
            </table>
        </div>
...
<script>
    function sortColumn(el) {
        ...
        fetch(`{{ route('crd.sapxep') }}?column=${column}&direction=${direction}`)
