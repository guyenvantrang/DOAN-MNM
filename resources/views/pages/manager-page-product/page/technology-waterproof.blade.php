...
@section('content')
    <div class="p-6 w-full">
        ...
        <h1 class="text-2xl font-semibold mb-6">Quản lý công nghệ chống nước</h1>

        {{-- Tìm kiếm + lọc ngày + reset --}}
        <div class="flex flex-col sm:flex-row gap-3 mb-6 items-start sm:items-center">
            <div class="flex-1 flex gap-3">
                <div class="relative w-full sm:w-1/3">
                    <input type="text" id="search" data-url-search="{{ url('congnghe/cn/timkiemtheomavaten') }}"
                        data-url-date="{{ url('congnghe/cn/timkiemtheongay') }}"
                        data-url-all="{{ url('congnghe/cn/hienthitatca') }}" placeholder="Tìm kiếm công nghệ..."
            ...
            <div x-data="{ modalOpen: false }">
                ...
                {{-- Modal thêm công nghệ --}}
                @include('components.message-box.technology-waterproof-add')
            </div>
        </div>

        {{-- Bảng công nghệ chống nước --}}
        <div x-data="categoryTable()" class="overflow-x-auto shadow-md sm:rounded-lg w-full">
            <table class="w-full text-sm text-left text-gray-100">
                <thead class="text-xs uppercase bg-gray-700 text-gray-200">
                    <tr>
                        <th class="px-4 py-3 cursor-pointer select-none" data-column="MCN" data-direction="asc"
                            ...
                            <div class="inline-flex items-center gap-2 group">
                                ...
                                <span class="text-gray-200 group-hover:text-blue-400 transition-colors">Mã công nghệ</span>
                                <span
                                    class="sort-arrow inline-block transform transition-transform duration-300 text-gray-400">↑</span>
                            </div>
                        </th>

                        <th class="px-4 py-3 cursor-pointer select-none" data-column="TEN" data-direction="asc"
                            onclick="sortColumn(this)">
                            <div class="inline-flex items-center gap-2 group">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                ...
                                <span class="text-gray-200 group-hover:text-blue-400 transition-colors">Tên công nghệ</span>
                                <span
                                    class="sort-arrow inline-block transform transition-transform duration-300 text-gray-400">↑</span>
                            </div>
                        </th>
                ...
                <tbody id="category-table" class="bg-gray-800">
                    @include('pages.manager-page-product.components.technology-waterproof-table', ['congnghe' => $congnghe])
                </tbody>
            </table>
        </div>
...
<script>
    function sortColumn(el) {
        ...
        fetch(`{{ route('cn.sapxep') }}?column=${column}&direction=${direction}`)
