...
@section('content')
    <div class="p-6 w-full">
        ...
        <h1 class="text-2xl font-semibold  mb-6">Quản lý thương hiệu</h1>

        {{-- Tìm kiếm + lọc ngày + reset --}}
        <div class="flex flex-col sm:flex-row gap-3 mb-6 items-start sm:items-center">
            <div class="flex-1 flex gap-3">
                <div class="relative w-full sm:w-1/3">
                    <input type="text" id="search" data-url-search="{{ url('thuonghieu/thuonghieu_sp/timkiemtheomavaten') }}"
                        data-url-date="{{ url('thuonghieu/thuonghieu_sp/timkiemtheongay') }}"
                        data-url-all="{{ url('thuonghieu/thuonghieu_sp/hienthitatca') }}" placeholder="Tìm kiếm loại sản phẩm..."
            ...
            <div x-data="{ modalOpen: false }">
                ...
                @include('components.message-box.brand-add')
            </div>
        </div>
        ...
        <div x-data="categoryTable()" class="overflow-x-auto shadow-md sm:rounded-lg w-full">
            <table class="w-full text-sm text-left text-gray-100">
                <thead class="text-xs uppercase bg-gray-700 text-gray-200">
                    <tr>
                        <th class="px-4 py-3 cursor-pointer select-none" data-column="MALOAI" data-direction="asc"
                            ...
                            <div class="inline-flex items-center gap-2 group">
                                ...
                                <span class="text-gray-200 group-hover:text-blue-400 transition-colors">Mã thương hiệu</span>
                                <span
                                    class="sort-arrow inline-block transform transition-transform duration-300 text-gray-400">↑</span>
                            </div>
                        </th>

                        <th class="px-4 py-3 cursor-pointer select-none" data-column="TENLOAI" data-direction="asc"
                            onclick="sortColumn(this)">
                            <div class="inline-flex items-center gap-2 group">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                ...
                                <span class="text-gray-200 group-hover:text-blue-400 transition-colors">Tên thương hiệu</span>
                                <span
                                    class="sort-arrow inline-block transform transition-transform duration-300 text-gray-400">↑</span>
                            </div>
                        </th>
                        ...
                        <th class="px-4 py-3 cursor-pointer select-none" data-column="MOTA" data-direction="asc"
                            ...
                            <div class="inline-flex items-center gap-2 group">
                                ...
                                <span class="text-gray-200 group-hover:text-blue-400 transition-colors">Quốc gia</span>
                                <span
                                    class="sort-arrow inline-block transform transition-transform duration-300 text-gray-400">↑</span>
                            </div>
                        </th>
                ...
                <tbody id="category-table" class="bg-gray-800">
                    @include('pages.manager-page-product.components.brand-table', ['thuonghieu' => $thuonghieu])
                </tbody>
            </table>
        </div>
...
<script>
    function sortColumn(el) {
        ...
        fetch(`{{ route('thuonghieu-sp.sapxeptheoid') }}?column=${column}&direction=${direction}`)
